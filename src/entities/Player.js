...
class Player {
  constructor(world) {
    ...
    this.chopTimer = 0;
    this.chopInterval = 1.2; // seconds per chop hit
    this.axeAngle = 0;       // spinning axe orbit angle (radians)

    // Mining
    this.mineTarget = null;
    this.mineTimer = 0;
    this.mineInterval = 1.5;

    // Crafting
    this.craftTarget = null;
    this.craftTimer = 0;
    this.craftInterval = 1.8;
  ...
  _beginCrafting(recipe, quantity, station) {
    this.state = PlayerState.CRAFTING;
    this.craftTarget = { recipe, station };
    this.craftQuantity = quantity;
    this.craftTimer = this.craftInterval; // Start immediately
    this.chopTarget = null;
    this.mineTarget = null;
  }
  ...
  update(dt) {
    ...
    } else if (this.state === PlayerState.CRAFTING) {
      this._updateCrafting(dt);
    }
  ...
  _cancelGatheringActions() {
    ...
    if (this.state === PlayerState.CHOPPING || this.state === PlayerState.MINING || this.state === PlayerState.CRAFTING) {
      this.state = PlayerState.IDLE;
    }
  }
  ...
  _updateCombat(dt) {
    ...
    if (this.state === PlayerState.CHOPPING || this.state === PlayerState.MINING || this.state === PlayerState.CRAFTING) return;
  ...
  addFloatingText(text, color = '#ffffff', yOffset = 0) {
    this.floatingTexts.push({ text, color, life: 1.5, yOffset });
  }
  ...
  render(ctx, camera) {
    ...
    for (const ft of this.floatingTexts) {
      const progress = 1 - ft.life / 1.5;
      const textY = by - 10 - ft.yOffset - (progress * 20);
      ctx.globalAlpha = ft.life > 0.5 ? 1 : ft.life * 2;
      ctx.fillStyle = '#000000';
      ctx.fillText(ft.text, sx + 1, textY + 1);
      ctx.fillStyle = ft.color;
      ctx.fillText(ft.text, sx, textY);
      ctx.globalAlpha = 1;
    }
